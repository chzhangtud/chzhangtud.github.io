...
test('merges CN, HK, MO, and TW counts into one CN row using the China flag', () => {
  assert.deepEqual(normalizeCountries(parseFlagCounterCountries(fixture)), [
    { sourceCode: 'cn', name: 'China', visitors: 18, label: 'CN', flagCode: 'cn' },
    { sourceCode: 'jp', name: 'Japan', visitors: 1, label: 'JP', flagCode: 'jp' },
  ]);
});

test('keeps ordinary countries unchanged', () => {
  assert.deepEqual(normalizeCountries([{ sourceCode: 'jp', name: 'Japan', visitors: 1 }]), [
    { sourceCode: 'jp', name: 'Japan', visitors: 1, label: 'JP', flagCode: 'jp' },
  ]);
});
...
test('serves normalized country data with CORS', async () => {
  ...
  assert.deepEqual((await response.json()).countries, [
    { sourceCode: 'cn', name: 'China', visitors: 18, label: 'CN', flagCode: 'cn' },
    { sourceCode: 'jp', name: 'Japan', visitors: 1, label: 'JP', flagCode: 'jp' },
  ]);
});
